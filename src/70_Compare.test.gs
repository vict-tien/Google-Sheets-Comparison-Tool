 ...
 * is compared. Test 18 lists HVAC BEFORE Rates for exactly that reason - a
...
function t_compare_tests() {
  ...
  t_test(26, 'Chain HVAC -> Assumptions -> Rates with a row inserted in ' +
    ...
    const A = t_workbook({
      HVAC:        t_plant(16, [[9, 2, '=Assumptions!$C$7',
                                      '=Assumptions!R7C3', 40]]),
      Assumptions: t_plant(16, [[6, 2, '=Rates!$B$4', '=Rates!R4C2', 40]]),
      Rates:       t_sheet(t_filler(16))
    });
    const B = t_workbook({
      HVAC:        t_plant(16, [[9, 2, '=Assumptions!$C$8',
                                      '=Assumptions!R8C3', 40]]),
